...
def build_drawings(bars):
    ...
    for i in range(len(segs)-2):
        ...
        # 의미있는 상승: 뒤 하락봉(B_t1)의 저가가 앞 하락봉(A_t1)의 저가보다 높아야 (저점 상승)
        if bars[B['t1']]['l'] <= bars[A['t1']]['l']: continue
    ...
    for i in range(len(segs)-2):
        ...
        # 의미있는 하락: 뒤 상승봉(B_t1)의 고가가 앞 상승봉(A_t1)의 고가보다 낮아야 (고점 하락)
        if bars[B['t1']]['h'] >= bars[A['t1']]['h']: continue
        ya1=MA2[A['t1']]; ta1=A['t1']; yb1=MA2[B['t1']]; tb1=B['t1']
        denom=sa-sb
        if abs(denom)<1e-9: continue
        xc=(yb1-ya1-sb*tb1+sa*ta1)/denom
        ...
        try:
            ...
            if 'draws' in mm and mm.get('chart'):
                ...
                try:
                    ...
                    mm['projection'] = None
            # all_risks 정리(용량) — 공방선(마디)용으로 yc만 추려 남기고 원본은 제거
            for blk in (dd,mm,tt):
                if isinstance(blk,dict):
                    ar=blk.get('all_risks') or []
                    blk['risk_nodes']=[round(r['yc'],2) for r in ar if r.get('yc',0)>0]
